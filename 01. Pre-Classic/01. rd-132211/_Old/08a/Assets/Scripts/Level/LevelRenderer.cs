        ...
        for(int x = 0; x < this.xChunks; x++) {
            for(int y = 0; y < this.yChunks; y++) {
                for(int z = 0; z < this.zChunks; z++) {
                    ...
                    newChunk.layer = LayerMask.NameToLayer("Ground");

                    //Chunk chunk = newChunk.AddComponent<Chunk>();

                    // Crie e associe o Chunk ao objeto de jogo do chunk
                    this.chunks[(x + y * this.xChunks) * this.zChunks + z] = new Chunk(level, x0, y0, z0, x1, y1, z1, meshFilter, meshRenderer, meshCollider);
                }
            }
        }
    }
